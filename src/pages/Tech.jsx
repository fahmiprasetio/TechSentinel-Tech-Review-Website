...
export default function TechPage() {
  ...
  const fetchTechnologies = async () => {
    try {
      ...
      setAllTechnologies(produk);
      console.log("🔥 DATA DARI API:", data); // log isi response lengkap
      console.log("✅ Jumlah produk dari API (data.data.length):", data.data?.length);
      console.log("📦 Daftar produk dari API:");
    } catch (err) {
  ...
  const filteredTechs = allTechnologies.filter((tech) => {
    console.log("💰 Harga:", tech.specs.price, typeof tech.specs.price);

    if (category && tech.category?.category_name?.toLowerCase() !== category.toLowerCase()) return false;
    if (filters.brand && tech.brand !== filters.brand) return false;

    if (filters.ram) {
      const ramValue = parseInt(tech.specs.ram?.replace(/\D/g, "")) || 0;
      if (filters.ram === ">24 GB") {
        if (ramValue <= 24) return false;
      } else {
        if (tech.specs.ram !== filters.ram) return false;
      }
    }

    const storageGB = parseInt(tech.specs.storage?.replace(/\D/g, "")) || 0;
    if (filters.storage) {
      if (filters.storage === "<64" && storageGB >= 64) return false;
      if (filters.storage === "64-128" && (storageGB < 64 || storageGB > 128))
        return false;
      if (filters.storage === "128-256" && (storageGB < 128 || storageGB > 256))
        return false;
      if (filters.storage === ">256" && storageGB <= 256) return false;
    }


    const price = Number(tech.specs?.[0]?.price || 0);
    console.log(`💰 Harga produk ${tech.tech_name}:`, price);

const minPrice = parseInt(filters.priceMin || "0");
const maxPrice = parseInt(filters.priceMax || "0");

if (filters.priceMin && price < minPrice) return false;
if (filters.priceMax && price > maxPrice) return false;
console.log("🧾 Filter harga aktif:", minPrice, "-", maxPrice);

    if (filters.rating && Math.floor(tech.rating) < parseInt(filters.rating))
      return false;

    if (
      searchTerm &&
      !tech.tech_name.toLowerCase().includes(searchTerm.toLowerCase())
    )
      return false;
...
const handlePriceInput = (field, value) => {
  ...
  setFilters({ ...filters, [field]: cleaned });
};
      ...
      {showFilter && (
        ...
        <div className="w-72 bg-[#1f2937] text-white shadow-lg p-4 border-r-2 border-r-gray-200">
          ...
          <div className="mb-4 mt-5">
            ...
            <Listbox
              value={kategoriOptions.find((k) => k.value === category)}
            ...
            >
              <div className="relative">
                <Listbox.Button className="relative w-full cursor-pointer rounded-md bg-gray-800 text-white py-2 pl-3 pr-10 text-left shadow-md hover:shadow-[5px_8px_20px_rgba(0,0,0,0.5)] border border-gray-400 focus:outline-none focus:ring-2 focus:ring-white transition-all">
                  <span>
                    {kategoriOptions.find((k) => k.value === category)?.name ||
                      "Show All"}
                  </span>
          ...
          <div className="mb-4">
            ...
            <Listbox
              value={brandOptions.find((b) => b.value === filters.brand)}
              onChange={(val) => handleFilterChange("brand", val.value)}
            >
              <div className="relative">
                <Listbox.Button className="relative w-full cursor-pointer rounded-md bg-gray-800 text-white py-2 pl-3 pr-10 text-left shadow-md hover:shadow-[5px_8px_20px_rgba(0,0,0,0.5)] border border-gray-400 focus:outline-none focus:ring-2 focus:ring-white transition-all">
                  <span>
                    {brandOptions.find((b) => b.value === filters.brand)
                      ?.name || "Show All"}
                  </span>
                  <span className="absolute inset-y-0 right-0 flex items-center pr-2 pointer-events-none">
                    <ChevronDown className="w-4 h-4" />
          ...
          <div className="mb-4">
            ...
            <Listbox
              value={ramOptions.find((r) => r.value === filters.ram)}
              onChange={(val) => handleFilterChange("ram", val.value)}
            >
              <div className="relative">
                <Listbox.Button className="relative w-full cursor-pointer rounded-md bg-gray-800 text-white py-2 pl-3 pr-10 text-left shadow-md hover:shadow-[5px_8px_20px_rgba(0,0,0,0.5)] border border-gray-400 focus:outline-none focus:ring-2 focus:ring-white transition-all">
                  <span>
                    {ramOptions.find((r) => r.value === filters.ram)?.name ||
                      "Show All"}
                  </span>
                  <span className="absolute inset-y-0 right-0 flex items-center pr-2 pointer-events-none">
                    <ChevronDown className="w-4 h-4" />
          ...
          <div className="mb-4">
            ...
            <Listbox
              value={storageOptions.find((s) => s.value === filters.storage)}
              onChange={(val) => handleFilterChange("storage", val.value)}
            >
              <div className="relative">
                <Listbox.Button className="relative w-full cursor-pointer rounded-md bg-gray-800 text-white py-2 pl-3 pr-10 text-left shadow-md hover:shadow-[5px_8px_20px_rgba(0,0,0,0.5)] border border-gray-400 focus:outline-none focus:ring-2 focus:ring-white transition-all">
                  <span>
                    {storageOptions.find((s) => s.value === filters.storage)
      ...
      <div className="flex-1 p-6">
        ...
        >
          {filteredTechs.map((tech) => (
            <div
              key={tech.id_tech}
              className="group border border-white/30 rounded-lg p-4 text-white shadow-md bg-white/10 transition duration-300 ease-in-out hover:shadow-2xl backdrop-blur-md"
              // style={{
            ...
            >
              ...
              />
              <h2
                className="text-lg ml-3 font-bold font-poppins mx-auto"
                style={{
              ...
              <p className="ml-3">
                ...
                IDR{" "}
                {tech.specs?.[0]?.price
                  ? Number(tech.specs[0].price).toLocaleString()
                  : "N/A"}
              </p>
              ...
              <Link
                ...
                className="mt-4 mb-3 ml-3 inline-block bg-transparent border border-white/60 text-white px-4 py-2 rounded-xl hover:bg-white hover:text-black transition"
              >
                View Details
              </Link>
